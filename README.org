...
*Figure 3* [[https://youtu.be/idlFzWeygwA][Video demo on v0.2.0 on YouTube]] featuring minor breaking changes and new transclusion filters
...
- [[#usage][Usage]]
  ...
  - [[#controlling-level-of-headlines-of-transclusions][Controlling level of headlines of transclusions]]
...
The transcluded text is *read-only* but you can copy it and export it as normal text. Org-transclusion remembers where it has transcluded the text from (its source buffer).  You can use a number of useful commands you can call by a single letter (by default). 
...
** Controlling level of headlines of transclusions
...
The top level of the transcluded headline will be set to the value of =:level= property -- in this example, level 2 regardless of that in the source. When the headline contains sub-headlines, all of them are automatically promoted or demoted to align to the top level headline.

When you transclude an entire Org file, it may contain multiple subtrees. In such cases, the top-most level among the subtrees will be set according to the =:level= property, and the rest of headlines in the buffer will align accordingly. 
