...
The ~#+transclusion~ keyword can be fontified via built-in ~font-lock-mode~ with
feature ~org-tranclusion-font-lock~. It is an extension that is turned on by
default, so you normally do not need any customizing. As an option for those who
