...
You can combine =:thing-at=point= with =:src= property so that the transcluded text is wrapped in Org's source block. Example 5 shows a way to do so. "id:123456789" is assumed to be present as comment in the source file.
...
- For =:thing-at-point, you cannot use :thing-at-point to specify the precise beginning of the thing within a line -- it is always the beginning of the line.
