...
This manual is for Org-transclusion version {{{version}}}. Version 2.0.0 has not
...
** (with version 2.0.0) Transient Menu
   ...
   same as the "single-letter-context-menu" (~org-transclusion-map)~ described in
   ...
   add "properties" to the =#+transclude= line. More on this later in the [[*Usage][Usage]]
...
- File link with =::dedicated-target=; this is intended for linking to a
  paragraph. See below.
- (with version 2.0.0) =::number= also works. =/regexp/= does not fail but does not
...
- (with version 2.0.0) ID link with a search option is also supported such as
  =id:uuid::*heading=
- File link for non-org files (tested with =.txt= and =.md=); for these, the whole
...
level of transcluded headlines to be one level deeper than the current headline.
...
When you specify a number, the top level of the transcluded headline will be set
to the value of =:level= property -- in this example, level 2 regardless of that
...
- (New with version 2.0.0) ~:no-first-heading~ ::
  ...
  headline titles from all the sutree. =:no-first-heading= removes only the first
...
- =:exclude-elements= ::
  This property lets you *add* elements to exclude per transclusion on top of the
  variable ~org-transclusion-exclude-elements~ defines. You cannot *remove* the ones
...
* (Version 2.0.0) Making Your Own Transcluding Functions
  ...
  Below is an example of ~orgit~ package, which enables Org to link to files in a
  certain commit in a Git repository. The link target is composed of the
  following elements. As long as you have a working Orgit package in your Emacs,
...
#+begin_example
#+transclude: [[orgit-file:~/src/org-transclusion/::0737cfe::org-transclusion.el::org-transclusion-new-function][org-transclusion-new-function (00737cfe)]]  :src emacs-lisp :thingatpt sexp
#+end_example
  ...
  local Maildir (via an email system named ~notmuch~: https://notmuchmail.org/ and
...
- Deactivating ~org-transclusion-font-lock-mode~ does not automatically re-fontify
  the Org buffers that you have already opened. Use ~revert-buffer~ or re-activate
  ~org-mode~ to enable the default Org fontification for keywords.

- Org link's search-options =::/regex/= does not fail but as it normally opens an
...
- ~org-transclusion-live-sync-start~ does not support all Org elements ::
  ...
  following elements: =center-block=, =drawer=, =dynamic-block=, =latex-environment=,
  ...
  =comment-block=, =export-block=, =example-block=, =fixed-width=, =keyword=, =src-block=,
  and =property-drawer=.
...
- Doom's customization may interfere with Org-transclusion ::
  Refer to issue [[https://github.com/nobiot/org-transclusion/issues/52][#52]]. The symptom is that in Doom you get an error message that
...
- Org refile does not work "properly" on the transcluded headlines ::
  Refer to issue [[https://github.com/nobiot/org-transclusion/issues/20][#20]]. I don't intend to support this -- refile the source, not
...
- For =:thing-at-point= or =:thingatpt=, you cannot use them to specify the precise
